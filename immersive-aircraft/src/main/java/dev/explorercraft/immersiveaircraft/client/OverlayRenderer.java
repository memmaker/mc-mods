...
import dev.explorercraft.immersiveaircraft.client.hud.*;
...
import static dev.explorercraft.immersiveaircraft.client.hud.Colors.colorFG;
...
public class OverlayRenderer {
    ...
    private void renderAircraftGui(Minecraft client, GuiGraphicsExtractor context, float tickDelta, EngineVehicle aircraft) {
        ...
        if (aircraft.getGuiStyle() == EngineVehicle.GUI_STYLE.ENGINE) {
            ...
            int halfAlphaWhite = ARGB.color(127, 255, 255, 255);
            context.blit(RenderPipelines.GUI_TEXTURED, POWER_TEX, x - 9, y - 9, (powerFrameTarget % 5) * 18, Math.floorDiv(powerFrameTarget, 5) * 18, 18, 18, 90, 90, halfAlphaWhite);
        }
    }
    ...
    // ponytail: GuiGraphics#renderOutline was dropped from GuiGraphicsExtractor; reimplemented as
    // 4 border fills (the old vanilla implementation), only needed by VectorIndicator's tiny
    // flight-director boxes.

...
public class PlushBootsGameTest {
    ...
    public void plushBootsRemoveFallDamage(GameTestHelper helper) {
        ...
        shod.setItemSlot(EquipmentSlot.FEET, new ItemStack(PlushBoots.PLUSH_BOOTS));
    ...
    public void woolCraftsIntoBootsOfThatColour(GameTestHelper helper) {
        for (DyeColor color : DyeColor.values()) {
            ...
            if (!crafted.is(PlushBoots.PLUSH_BOOTS)) {
    ...
    public void bootsTakeDye(GameTestHelper helper) {
        ItemStack dyed = craft(helper, 2, 1,
                List.of(new ItemStack(PlushBoots.PLUSH_BOOTS), new ItemStack(Items.DYE.pick(DyeColor.LIME))));

        if (!dyed.is(PlushBoots.PLUSH_BOOTS)) {
    ...
    public void debugCommandGivesBoots(GameTestHelper helper) {
        ...
        if (server.getCommands().getDispatcher().getRoot().getChild(PlushBoots.MOD_ID) == null) {
            throw helper.assertionException("/" + PlushBoots.MOD_ID + " is not registered");
        }

        var player = (ServerPlayer) helper.makeMockServerPlayer(GameType.CREATIVE);
        server.getCommands().performPrefixedCommand(
                server.createCommandSourceStack().withEntity(player), PlushBoots.MOD_ID + " give");

        ItemStack given = player.getInventory().getItem(0);

        if (given.getItem() != PlushBoots.PLUSH_BOOTS) {

...
import dev.explorercraft.immersiveaircraft.Items;
...
public class GyrodyneEntity extends Rotorcraft {
    ...
    protected float getCustomGravity() {
        return (1.0f - getEnginePower()) * super.getCustomGravity();
    }
    ...
    private void updateEnginePowerTooltip() {
        if (getControllingPassenger() instanceof Player player && player.level().isClientSide() && getFuelUtilization() > 0.0) {
            player.sendOverlayMessage(Component.translatable("immersiveaircraft.gyrodyne_target", (int) (getEngineTarget() * 100.f + 0.5f)));
        }
    }
        ...
        if (getEngineTarget() < 1.0f) {
            ...
            if (getEngineTarget() == 1.0) {
                if (getControllingPassenger() instanceof Player player) {
                    player.sendOverlayMessage(Component.translatable("immersiveaircraft.gyrodyne_target_reached"));
                }
                if (onGround()) {

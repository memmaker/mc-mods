...
import dev.explorercraft.immersiveaircraft.network.c2s.EnginePowerMessage;
...
public abstract class EngineVehicle extends InventoryVehicleEntity {
                ...
                if (getControllingPassenger() instanceof ServerPlayer player) {
                    ...
                    if (utilization > 0 && isFuelLow()) {
                        if (lastFuelState != FuelState.LOW) {
                            player.sendOverlayMessage(Component.translatable("immersiveaircraft." + getFuelType() + ".low"));
                        ...
                        if (lastFuelState != FuelState.EMPTY) {
                            player.sendOverlayMessage(Component.translatable("immersiveaircraft." + getFuelType() + "." + (lastFuelState == FuelState.FUELED ? "out" : "none")));
    ...
    private void refuel(int i) {
        ...
        while (fuel[i] <= TARGET_FUEL && i < slots.size()) {
            ...
            int time = Utils.getFuelTime(level(), stack);
            if (time > 0) {
                fuel[i] += time;

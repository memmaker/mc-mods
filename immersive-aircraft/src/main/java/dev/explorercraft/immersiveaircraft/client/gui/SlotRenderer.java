...
public class SlotRenderer {
    ...
    static {
        register("inventory", (screen, graphics, slot, mouseX, mouseY, delta) ->
                screen.drawImage(graphics, screen.getX() + slot.x() - 1, screen.getY() + slot.y() - 1, 284, 0, 18, 18));
        register("default", (screen, graphics, slot, mouseX, mouseY, delta) ->
                screen.drawImage(graphics, screen.getX() + slot.x() - 3, screen.getY() + slot.y() - 3, 262, 0, 22, 22));
        register("boiler", (screen, graphics, slot, mouseX, mouseY, delta) -> {
            screen.drawImage(graphics, screen.getX() + slot.x() - 4, screen.getY() + slot.y() - 18, 318, 0, 24, 39);
            if (screen.getMenu().getVehicle() instanceof EngineVehicle engineAircraft && engineAircraft.getFuelUtilization() > 0.0) {
                screen.drawImage(graphics, screen.getX() + slot.x() - 4, screen.getY() + slot.y() - 18, 318 + 30, 0, 24, 39);
            }
        });
    ...
    public static class BasicRenderer implements Renderer {
        ...
        public void render(VehicleScreen screen, @NotNull GuiGraphicsExtractor graphics, SlotDescription slot, int mouseX, int mouseY, float delta) {
            if (screen.getMenu().getVehicle().getInventory().getItem(slot.index()).isEmpty()) {
                screen.drawImage(graphics, screen.getX() + slot.x() - 3, screen.getY() + slot.y() - 3, u, v, 22, 22);
            } else {
                screen.drawImage(graphics, screen.getX() + slot.x() - 3, screen.getY() + slot.y() - 3, 262, 0, 22, 22);
            }
        }
    ...
    public interface Renderer {
        void render(VehicleScreen screen, @NotNull GuiGraphicsExtractor graphics, SlotDescription slot, int mouseX, int mouseY, float delta);
    }
}

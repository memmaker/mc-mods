...
import dev.explorercraft.immersiveaircraft.cobalt.registration.CobaltFuelRegistry;
...
public class Utils {
    ...
    public static int getFuelTime(Level level, ItemStack fuel) {
        if (fuel.isEmpty()) {
            return 0;
        }
        ...
        if (Config.getInstance().acceptVanillaFuel) {
            int fuelTime = CobaltFuelRegistry.INSTANCE.get(level, fuel);
            if (fuelTime > 0) {
                return fuelTime;
            }
        }
